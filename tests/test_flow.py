...
class FlowProvider:
    ...
    async def complete(self, model: str, request: CanonicalRequest) -> Completion:
        ...
        if "request assessment agent" in system:
        ...
        elif "Review the proposed answer" in system:
            if self.investigate:
                return Completion(
            ...
            content = '{"needs_investigation": false}'
...
async def test_investigation_result_integrates_then_checks_again() -> None:
    ...
    assert "Review the proposed answer" in provider.requests[1][1].messages[0]["content"]
...
@pytest.mark.asyncio
async def test_aggregate_client_action_is_validated_and_skips_checker() -> None:
    read_tool = {
        "type": "function",
        "function": {
            "name": "read",
    ...
    assert "Review the proposed answer" not in str(provider.requests[-1][1].messages[0])
...
async def test_gate_deadline_includes_time_waiting_for_concurrency() -> None:
    class SlowProvider(FlowProvider):
        async def complete(self, model: str, request: CanonicalRequest) -> Completion:
            system = str(request.messages[0].get("content") or "")
            if "CONTRIBUTOR COUNCIL" in system:
...
@pytest.mark.asyncio
async def test_investigation_limit_removes_task_from_checker() -> None:
    provider = FlowProvider()
    gateway = flow_gateway(provider)
    calls = [
        {
